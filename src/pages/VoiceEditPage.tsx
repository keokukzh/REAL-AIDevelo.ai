...
export const VoiceEditPage = () => {
  ...
  const voiceId =
    voiceIdParam && !voiceIdParam.startsWith('voice_') && voiceIdParam.length > 10
      ? voiceIdParam
      : null;
  ...
  const handleGenerateTest = async () => {
    ...
    try {
      ...
      const response = await apiRequest<{
        success: boolean;
        data: { audioUrl?: string; audioBase64?: string };
      ...
      if (error instanceof ApiRequestError) {
        ...
        if (
          ...
          const errorMsg =
            error.message.includes('ELEVENLABS_API_KEY') || error.message.includes('API key')
    ...
    <div className="min-h-screen bg-background text-white flex flex-col">
      ...
      <header className="p-6 border-b border-white/10 flex items-center justify-between bg-surface/50 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" onClick={handleRerecord} className="text-sm">
            <RefreshCw size={16} className="mr-2" />
            Neu aufnehmen
          </Button>
          <Button variant="primary" onClick={handleSaveAndContinue} className="text-sm">
        ...
        >
          ...
          <div className="bg-accent/10 border border-accent/30 rounded-2xl p-6">
            ...
            <ul className="space-y-2 text-gray-300">
              <li>
                • Testen Sie verschiedene Texte, um die Qualität Ihrer geklonten Stimme zu
                überprüfen
              </li>
