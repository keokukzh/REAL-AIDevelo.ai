...
import { ConfigurationTab } from '../components/agent/ConfigurationTab.js';
import { RAGManagementTab } from '../components/agent/RAGManagementTab.js';
import { AnalyticsTab } from '../components/agent/AnalyticsTab.js';
import { CallHistoryTab } from '../components/agent/CallHistoryTab.js';
...
export const AgentDetailsPage = () => {
  ...
  const handleTestAgent = () => {
    // Updated to use internal test call instead of external link
    navigate(ROUTES.TEST_CALL);
  };
  ...
  const getStatusBadge = (status: Agent['status']) => {
    ...
    return (
      <div
        className={`px-3 py-1.5 rounded-full text-sm font-bold flex items-center gap-2 ${badge.color}`}
      >
        <Icon size={16} />
        {badge.text}
      </div>
    ...
    <div className="min-h-screen bg-background text-white">
      <header className="p-6 border-b border-white/10 flex items-center justify-between bg-surface/50 backdrop-blur-md sticky top-0 z-50 relative">
        <div className="flex-1 flex items-center gap-4">
          ...
          <Breadcrumb
            items={breadcrumbs}
            currentLabel={agent.businessProfile.companyName || 'Agent Details'}
        ...
        <div className="flex-1 flex justify-center">
          ...
          />
          <span
            className="logo-fallback-text hidden font-display font-bold text-xl cursor-pointer"
            onClick={() => navigate('/dashboard')}
          >
            AIDevelo Studio
          </span>
      ...
      <main className="container mx-auto p-6 md:p-12 max-w-7xl">
        <div className="mb-8">
          <h1 className="text-3xl font-bold font-display mb-2">
            {agent.businessProfile.companyName}
          </h1>
        ...
        <div className="mt-6">
          ...
          {activeTab === 'rag' && id && <RAGManagementTab agentId={id} />}
          {activeTab === 'analytics' && id && <AnalyticsTab agentId={id} />}
          {activeTab === 'calls' && id && <CallHistoryTab agentId={id} />}
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="font-medium">
                {agent.businessProfile.location.city}, {agent.businessProfile.location.country}
              </p>
            </div>
            <div>
            ...
            {agent.businessProfile.website && (
              <div>
                ...
                <p className="font-medium flex items-center gap-2">
                  <Globe size={14} />
                  <a
                    href={agent.businessProfile.website}
                    target="_blank"
        ...
        >
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="font-medium font-mono text-sm">{agent.config.voiceSettings.voiceId}</p>
            </div>
            <div>
              <span className="text-gray-400 text-sm">Modell:</span>
              <p className="font-medium">{agent.config.voiceSettings.modelId}</p>
            </div>
          </div>
        </motion.div>
      </div>
