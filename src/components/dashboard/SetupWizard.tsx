  ...
  React.useEffect(() => {
    if (overview?.agent_config) {
      setFormData({
        ...
        company_name: overview.agent_config.company_name || '',
        goals_json: Array.isArray(overview.agent_config.goals_json)
          ? overview.agent_config.goals_json
          : [],
        services_json: Array.isArray(overview.agent_config.services_json)
          ? overview.agent_config.services_json
          : [],
      });
      ...
      if (state === 'needs_persona') setCurrentStep('persona');
        ...
        if (
          overview.agent_config.services_json &&
          Array.isArray(overview.agent_config.services_json) &&
          overview.agent_config.services_json.length > 0
        ) {
    ...
    <div className="bg-gray-800 rounded-lg p-6 mb-8">
      <div className="mb-6">
        ...
        <div className="flex gap-2 mb-4">
          {(['persona', 'business', 'services', 'goals', 'confirm'] as WizardStep[]).map(
            (step, idx) => (
              <div
                key={step}
                className={`flex-1 h-2 rounded ${
                  idx <=
                  (['persona', 'business', 'services', 'goals', 'confirm'] as WizardStep[]).indexOf(
                    currentStep,
                  )
                    ? 'bg-accent'
                    : 'bg-gray-700'
        ...
        <>
          ...
          {currentStep === 'persona' && (
            <div>
              ...
              <p className="text-gray-400 mb-6">
                Wähle Stimme & Auftreten. Du kannst das jederzeit ändern.
              </p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        persona_gender: e.target.value as 'male' | 'female',
                      })
                    }
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, persona_age_range: e.target.value })
                    }
                    className="w-full px-4 py-2 bg-gray-700 rounded text-white"
                  >
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewService({ ...newService, durationMin: parseInt(e.target.value) || 30 })
                    }
          ...
          {currentStep === 'confirm' && (
            <div>
              ...
              <p className="text-gray-400 mb-6">
                Überprüfe deine Einstellungen und schließe die Einrichtung ab.
              </p>

              <div className="bg-gray-700 rounded p-4 space-y-2">
                <div>
                  <strong>Persona:</strong>{' '}
                  {formData.persona_gender === 'female' ? 'Weiblich' : 'Männlich'},{' '}
                  {formData.persona_age_range} Jahre
                </div>
                <div>
                  <strong>Business Typ:</strong> {formData.business_type}
                </div>
                <div>
                  <strong>Services:</strong> {formData.services_json.length} Leistungen
                </div>
                <div>
                  <strong>Ziele:</strong> {formData.goals_json.length} Ziele
                </div>
              </div>
            </div>
          )}
