...
export const SplitPortal: React.FC = () => {
  ...
  const sides = [
    ...
    },
    {
      ...
      bgImage:
        'https://images.unsplash.com/photo-1589254065878-42c9da997008?auto=format&fit=crop&q=80&w=2070',
        ...
        {sides.map((side) => {
          ...
          return (
            ...
            >
              ...
              >
                <img src={side.bgImage} alt={side.title} className="w-full h-full object-cover" />
              </motion.div>

              {/* Dynamic Gradient Overlay */}
              <div
                className={`absolute inset-0 z-10 bg-gradient-to-t via-black/40 to-transparent ${isHovered ? 'opacity-90' : 'opacity-60'} transition-opacity duration-700`}
              />
              <div
                className={`absolute inset-0 z-10 bg-gradient-to-r ${side.id === 'webdesign' ? 'from-black/80 to-transparent' : 'from-transparent to-black/80'} opacity-60`}
              />
              ...
              {/* Content Container */}
              <div
                className={`relative z-20 h-full flex flex-col items-center justify-center p-8 md:p-16 text-center transition-all duration-700 ${isHovered ? 'translate-y-0 opacity-100' : 'translate-y-4 md:translate-y-0 opacity-80'}`}
              >
                ...
                {side.id === 'webdesign' && isHovered && (
                  <div
                    ...
                    style={{
                      background:
                        'radial-gradient(circle, rgba(218,41,28,0.2) 1px, transparent 1px)',
                ...
                >
                  <side.icon
                    className={`w-12 h-12 md:w-16 md:h-16 ${side.id === 'webdesign' ? 'text-swiss-red' : 'text-cyan-400'}`}
                  />
                  {isHovered && (
                    <motion.div
                      layoutId="glow"
                ...
                >
                  {side.features.map((feature, i) => (
                    ...
                    >
                      <Sparkles
                        size={14}
                        className={side.id === 'webdesign' ? 'text-swiss-red' : 'text-cyan-400'}
                      />
                      {feature}
                    </div>
                  ))}
