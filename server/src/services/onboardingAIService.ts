...
export const onboardingAIService = {
  ...
  buildSystemPrompt(context: AssistantContext): string {
    const taskDescriptions: Record<string, string> = {
      ...
      hours:
        'Der Benutzer legt die Öffnungszeiten fest, wann der Agent Anrufe entgegennehmen soll.',
      goals:
        'Der Benutzer definiert die Ziele des Voice Agents (z.B. Terminbuchung, Lead-Qualifizierung).',
...
Antworte kurz und prägnant (max. 3-4 Sätze).`;
  ...
  getMockResponse(context: AssistantContext): string {
    const responses: Record<string, string> = {
      company:
        'Geben Sie bitte den Firmennamen und eine E-Mail-Adresse ein. Diese Informationen werden für die Konfiguration Ihres Voice Agents benötigt.',
      hours:
        'Sie können wählen, ob der Agent 24/7 erreichbar sein soll oder nur ausserhalb der Geschäftszeiten. 24/7 wird empfohlen, um keine Anrufe zu verpassen.',
      goals:
        'Definieren Sie die Hauptaufgabe Ihres Agents. Sie können mehrere Ziele auswählen, z.B. Terminbuchung, Lead-Qualifizierung oder Support.',
      calendar:
        'Verbinden Sie Ihren Kalender (Google Calendar oder Outlook), damit der Agent automatisch Termine buchen kann.',
      voice:
        'Beim Voice Cloning nehmen Sie einige Sätze auf, damit der Agent in Ihrer Stimme sprechen kann.',
    };
