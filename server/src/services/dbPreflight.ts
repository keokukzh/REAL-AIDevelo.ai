...
export async function checkDbPreflight(): Promise<PreflightResult> {
  ...
  // Bypass preflight in dev mode if auth bypass is enabled
  if (process.env.DEV_BYPASS_AUTH === 'true') {
    return {
      ok: true,
      missing: [],
      warnings: ['Dev bypass enabled - skipping database checks'],
      projectUrl: 'dev-bypass',
  ...
  try {
    ...
    warnings.push(
      'Could not verify set_updated_at function - ensure schema.sql and security_hardening.sql are applied',
    );
  }
