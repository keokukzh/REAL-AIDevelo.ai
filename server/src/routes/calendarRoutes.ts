    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (
          ...
          errorMessage =
            'Google OAuth ist nicht konfiguriert. Bitte setze GOOGLE_OAUTH_CLIENT_ID in den Environment Variables.';
        }
      }
...
router.get('/:provider/callback', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (stateData.provider !== provider) {
      return next(
        new BadRequestError(`Provider mismatch: expected ${provider}, got ${stateData.provider}`),
      );
    }
    ...
    console.log(
      `[CalendarRoutes] Stored token for locationId=${stateData.locationId}, provider=${provider}`,
    );
    ...
    if (!frontendUrl || frontendUrl === '') {
      if (process.env.NODE_ENV === 'production') {
        ...
        console.warn(
          '[CalendarRoutes] FRONTEND_URL not set, using https://aidevelo.ai as fallback',
        );
        frontendUrl = 'https://aidevelo.ai';
      } else {
        // Dev fallback: use '*' for postMessage (less secure but allows testing)
        console.warn(
          '[CalendarRoutes] FRONTEND_URL not set, using "*" as postMessage targetOrigin (dev only)',
        );
        frontendUrl = '*';
      }
    }
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('TOKEN_ENCRYPTION_KEY')) {
        errorMessage =
          'TOKEN_ENCRYPTION_KEY fehlt oder ist ungültig. Bitte setze diese Variable in Render Environment Variables. Siehe docs/TOKEN_ENCRYPTION_KEY_SETUP.md';
      } else if (error.message.includes('refreshToken is required')) {
        errorMessage =
          'Google OAuth hat keinen Refresh Token zurückgegeben. Bitte versuche es erneut.';
        ...
        error.message.includes('exchangeGoogleCode') ||
        error.message.includes('exchangeOutlookCode')
      ) {
        ...
        errorMessage =
          'Fehler beim Speichern des Kalender-Tokens. Bitte überprüfe die Datenbankverbindung.';
      } else if (
        error.message.includes('GOOGLE_OAUTH_CLIENT_ID') ||
        error.message.includes('GOOGLE_OAUTH_CLIENT_SECRET')
      ) {
        errorMessage =
          'Google OAuth ist nicht konfiguriert. Bitte setze GOOGLE_OAUTH_CLIENT_ID und GOOGLE_OAUTH_CLIENT_SECRET in Render Environment Variables.';
      } else if (
        error.message.includes('Invalid state') ||
        error.message.includes('Ungültiger OAuth-State')
      ) {
      ...
      if (axiosError.status === 400) {
        ...
        errorMessage =
          'Google OAuth Authentifizierung fehlgeschlagen. Bitte überprüfe GOOGLE_OAUTH_CLIENT_ID und GOOGLE_OAUTH_CLIENT_SECRET.';
    ...
    const escapedMessage = errorMessage.replace(/'/g, "\\'").replace(/\n/g, ' ').replace(/\r/g, '');
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      console.log(
        `[CalendarRoutes] Disconnected calendar for locationId=${location.id}, provider=${provider}`,
      );
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      try {
        ...
        console.log(
          `[CalendarRoutes] Admin check-availability: resolved locationId=${locationId} from source=${resolution.source}`,
        );
      } catch (error: any) {
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      try {
        ...
        locationId = resolution.locationId;
        console.log(
          `[CalendarRoutes] Admin create-appointment: resolved locationId=${locationId} from source=${resolution.source}`,
        );
      } catch (error: any) {
        return res.status(400).json({
          success: false,
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      try {
        ...
        locationId = resolution.locationId;
        console.log(
          `[CalendarRoutes] List events: resolved locationId=${locationId} from source=${resolution.source}`,
        );
      } catch (error: any) {
        return res.status(400).json({
          success: false,
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      try {
        ...
        locationId = resolution.locationId;
        console.log(
          `[CalendarRoutes] Update event: resolved locationId=${locationId} from source=${resolution.source}`,
        );
      } catch (error: any) {
        return res.status(400).json({
          success: false,
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      try {
        ...
        locationId = resolution.locationId;
        console.log(
          `[CalendarRoutes] Delete event: resolved locationId=${locationId} from source=${resolution.source}`,
        );
      } catch (error: any) {
        return res.status(400).json({
          success: false,
