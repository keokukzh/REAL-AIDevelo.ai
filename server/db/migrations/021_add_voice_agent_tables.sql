...
-- Voice Profiles (per tenant/location) - only create if locations table exists
DO $$
BEGIN
...
-- Extend agent_configs (add voice_profile_id) - only if both tables exist
DO $$
BEGIN
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'agent_configs') THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'agent_configs' AND column_name = 'voice_profile_id') THEN
...
-- Updated_at trigger for agent_templates
